...
async function handleRenameGroup({ groupId, newName }) {
  ...
  } catch (error) { ElMessage.error('重命名失败，请检查输入') }
}
...
async function handleDeleteGroup({ groupId }) {
  ...
  } catch (error) { ElMessage.error('删除失败，请检查网络或日志') }
}
...
async function handleSaveStages({ baseStages: newBase, lifeStages: newLife }) {
  ...
  } catch (error) { ElMessage.error('保存失败，请检查控制台或网络错误') }
}
...
function confirmResult() {
  ...
  // 处理结局
  if (Number(selected.nextStageId) === -1) {
    phase.value = 'done'
  ...
  if (nextStageId) {
    ...
    if (baseTarget !== -1) {
      phase.value = 'base'; baseIndex.value = baseTarget; resetWheel(); return
    }
    const lifeTarget = lifeStages.value.findIndex(s => s.id === nextStageId)
    if (lifeTarget !== -1) {
      phase.value = 'life'; lifeIndex.value = lifeTarget; resetWheel(); return
    }
  }
  ...
  if (currentPhase === 'base') {
    ...
    if (baseIndex.value >= baseStages.value.length) {
      phase.value = 'life'; lifeIndex.value = 0
    }
  } else {
    lifeIndex.value++
...
function resetGame() {
  baseIndex.value = 0; lifeIndex.value = 0
  baseHistory.value = []; lifeHistory.value = []
  baseCurrentResult.value = null; lifeCurrentResult.value = null
...
watch(currentGroupId, (newVal) => {
  if (newVal) localStorage.setItem(STORAGE_KEY_GROUP, String(newVal))
  else localStorage.removeItem(STORAGE_KEY_GROUP)
})
...
h1 { color: #f5e6b0; text-shadow: 0 0 20px #f0b34b; }
.history-summary {
  width: 100%; background: #1f1525; border: 1px solid #4a3a3a;
  border-radius: 20px; padding: 10px 20px; margin-bottom: 15px;
  min-height: 50px; text-align: left; display: flex;
  align-items: center; justify-content: center;
}
.history-row { font-size: 1.1rem; font-weight: 500; }
.tag-base { color: #4a7a9a; font-weight: bold; margin-right: 8px; }
.top-section { margin: 10px 0 20px 0; display: flex; flex-direction: column; align-items: center; gap: 10px; }
.stage-header h2 { color: #f5e6b0; margin: 0; }
.controls-top { display: flex; gap: 15px; justify-content: center; flex-wrap: wrap; }
.game-stage { display: flex; justify-content: center; align-items: flex-start; gap: 30px; width: 100%; max-width: 800px; margin: 10px auto; flex-wrap: wrap; }
.bottom-section { margin-top: 20px; display: flex; flex-direction: column; align-items: center; gap: 15px; width: 100%; }
.result-area { display: flex; flex-direction: column; align-items: center; gap: 15px; }
.current-result { background: #1d1428; border: 1px solid #5f4a3a; border-radius: 20px; padding: 15px 25px; min-width: 200px; margin: 0 auto; }
.result-label { font-size: 1.4rem; font-weight: bold; color: #f5e6b0; }
.result-desc { font-size: 0.9rem; color: #bdaa88; }
.wheel-wrapper { position: relative; width: 100%; max-width: 400px; margin: 10px auto; }
.complete-msg { width: 100%; background: #2a1a1a; padding: 20px; border-radius: 20px; border: 2px solid #f5c542; color: #f5c542; font-size: 1.2rem; margin-top: 20px; }
.final-summary { margin: 15px 0; font-size: 1rem; color: #e5d4b0; }
.ending-text { margin-top: 15px; font-size: 1.1rem; color: #f5d78a; }
</style>